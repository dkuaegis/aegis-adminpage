...
export const usePointPageState = () => {
  ...
  const [ledgerData, setLedgerData] = useState<AdminPointLedgerPage | null>(
    null
  );
  ...
  const [ledgerTransactionType, setLedgerTransactionType] =
    useState<PointTransactionFilter>("ALL");
  const [ledgerSorting, setLedgerSorting] = useState<SortingState>(
    DEFAULT_LEDGER_SORTING
  );
  ...
  const [memberCache, setMemberCache] = useState<
    Record<number, AdminPointMemberSearch>
  >({});
  ...
  const [memberPoint, setMemberPoint] = useState<AdminPointMemberPoint | null>(
    null
  );
  ...
  const [selectedBatchMemberIds, setSelectedBatchMemberIds] = useState<
    number[]
  >([]);
  ...
  const [batchResult, setBatchResult] =
    useState<AdminPointBatchGrantResult | null>(null);
  ...
  const selectedBatchMemberIdSet = useMemo(
    () => new Set(selectedBatchMemberIds),
  ...
  const handleLedgerPaginationChange = (
  ...
  const handleBatchGrant = async (): Promise<void> => {
    ...
    const confirmed = await showConfirm(
      `선택된 ${selectedBatchMemberIds.length}명에게 일괄 지급하시겠습니까?`
    );
    if (!confirmed) {
      return;
    }
