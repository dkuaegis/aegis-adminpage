...
export const useEventPageState = (): UseEventPageStateResult => {
  const [activityPage, setActivityPage] =
    useState<AdminActivityPageResponse | null>(null);
  ...
  const loadActivities = useCallback(
    async (
      ...
      if (!response.ok) {
        ...
        showError(
          resolveAdminErrorMessage(response.errorName, {
            fallback: "활동 목록을 불러올 수 없습니다.",
          })
        );
        setIsLoading(false);
        return;
      ...
      if (!data) {
        setActivityPage(null);
        showError(
          resolveAdminErrorMessage(undefined, {
            fallback: "활동 목록을 불러올 수 없습니다.",
          })
        );
        setIsLoading(false);
  ...
  const handleSortingChange = useCallback((updater: Updater<SortingState>) => {
    setSorting((prev) =>
      normalizeSingleSorting(updater, prev, DEFAULT_EVENT_SORTING)
    );
    setPagination((prev) => ({
      ...prev,
      pageIndex: 0,
