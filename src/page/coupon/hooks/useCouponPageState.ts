...
const couponErrorOverrides: Record<string, string> = {
  COUPON_ALREADY_EXISTS:
    "동일한 이름과 할인 금액을 가진 쿠폰이 이미 존재합니다.",
  COUPON_NOT_FOUND: "쿠폰을 찾을 수 없습니다.",
  COUPON_ISSUED_COUPON_EXISTS:
    "발급된 쿠폰이 남아 있어 쿠폰을 삭제할 수 없습니다.",
  COUPON_CODE_ALREADY_USED_CANNOT_DELETE:
    "이미 사용된 쿠폰 코드는 삭제할 수 없습니다.",
  ISSUED_COUPON_ALREADY_USED:
    "이미 사용된 쿠폰 발급 내역은 삭제할 수 없습니다.",
  COUPON_CODE_CANNOT_ISSUE_CODE:
    "쿠폰 코드 생성에 실패했습니다. 잠시 후 다시 시도해주세요.",
};

const resolveCouponErrorMessage = (errorName?: string): string => {
  return resolveAdminErrorMessage(errorName, {
    overrides: couponErrorOverrides,
  });
};
...
export const useCouponPageState = (): CouponPageState => {
  ...
  const [couponPage, setCouponPage] = useState<AdminCouponPageResponse | null>(
    null
  );
  const [couponCodePage, setCouponCodePage] =
    useState<AdminCouponCodePageResponse | null>(null);
  const [issuedCouponPage, setIssuedCouponPage] =
    useState<AdminIssuedCouponPageResponse | null>(null);
  ...
  const [selectedCouponIdForCode, setSelectedCouponIdForCode] =
    useState<number>(0);
  const [newCouponCodeDescription, setNewCouponCodeDescription] = useState("");
  const [selectedCouponIdForIssue, setSelectedCouponIdForIssue] =
    useState<number>(0);

  const [couponSorting, setCouponSorting] = useState<SortingState>(
  ...
  const [couponCodeSorting, setCouponCodeSorting] = useState<SortingState>(
    DEFAULT_COUPON_CODE_SORTING
  );
  const [couponCodePagination, setCouponCodePagination] =
    useState<PaginationState>({
      pageIndex: 0,
      pageSize: PAGE_SIZE,
    });

  const [issuedSorting, setIssuedSorting] = useState<SortingState>(
    DEFAULT_ISSUED_SORTING
  );
  ...
  const fetchCouponOptions = useCallback(async (): Promise<void> => {
    ...
    setCouponNameDrafts((prevDrafts) => {
      ...
      data.content.forEach((coupon) => {
        nextDrafts[coupon.couponId] =
          prevDrafts[coupon.couponId] ?? coupon.couponName;
      });
      return nextDrafts;
    });
  }, []);
  ...
  const fetchCouponPage = useCallback(
    async (
  ...
  const fetchCouponCodePage = useCallback(
    async (
      pagination: PaginationState,
      sorting: SortingState,
      keyword: string
  ...
  const fetchIssuedCouponPage = useCallback(
    async (
      pagination: PaginationState,
      sorting: SortingState,
      keyword: string
  ...
  }, [fetchCouponOptions, fetchMembers]);
  ...
  useEffect(() => {
    ...
    if (
      !coupons.some((coupon) => coupon.couponId === selectedCouponIdForCode)
    ) {
      setSelectedCouponIdForCode(coupons[0].couponId);
    }

    if (
      !coupons.some((coupon) => coupon.couponId === selectedCouponIdForIssue)
    ) {
  ...
  useEffect(() => {
    ...
    tab,
    couponPagination.pageIndex,
    couponPagination.pageSize,
    couponSorting,
    appliedKeyword,
    couponPagination,
    fetchCouponPage,
  ...
  useEffect(() => {
    ...
    tab,
    couponCodePagination.pageIndex,
    couponCodePagination.pageSize,
    couponCodeSorting,
    appliedKeyword,
    couponCodePagination,
    fetchCouponCodePage,
  ...
  useEffect(() => {
    ...
    tab,
    issuedPagination.pageIndex,
    issuedPagination.pageSize,
    issuedSorting,
    appliedKeyword,
    fetchIssuedCouponPage,
    issuedPagination,
  ]);
  ...
  const handleCouponSortingChange = (updater: Updater<SortingState>): void => {
    setCouponSorting((prev) =>
      normalizeSingleSorting(updater, prev, DEFAULT_COUPON_SORTING)
    );
    setCouponPagination((prev) => ({
      ...prev,
      pageIndex: 0,
  ...
  const handleCouponPaginationChange = (
  ...
  const handleCouponCodeSortingChange = (
    updater: Updater<SortingState>
  ): void => {
    setCouponCodeSorting((prev) =>
      normalizeSingleSorting(updater, prev, DEFAULT_COUPON_CODE_SORTING)
    );
    setCouponCodePagination((prev) => ({
      ...prev,
      pageIndex: 0,
  ...
  const handleCouponCodePaginationChange = (
    updater: Updater<PaginationState>
  ): void => {
    ...
    if (
      couponCodePage &&
      nextPagination.pageIndex >= couponCodePage.totalPages
    ) {
      return;
    }
  ...
  const handleIssuedSortingChange = (updater: Updater<SortingState>): void => {
    setIssuedSorting((prev) =>
      normalizeSingleSorting(updater, prev, DEFAULT_ISSUED_SORTING)
    );
    setIssuedPagination((prev) => ({
      ...prev,
      pageIndex: 0,
  ...
  const handleIssuedPaginationChange = (
    updater: Updater<PaginationState>
  ): void => {
    ...
    if (
      issuedCouponPage &&
      nextPagination.pageIndex >= issuedCouponPage.totalPages
    ) {
      return;
    }
  ...
  const selectedMemberIdSet = useMemo(
    () => new Set(selectedMemberIdsForIssue),
  ...
  const isAllFilteredMembersSelected =
    ...
    filteredMembersForIssue.every((member) =>
      selectedMemberIdSet.has(member.memberId)
    );

  const selectedAmongFilteredCount = useMemo(() => {
    return selectedMemberIdsForIssue.filter((memberId) =>
      filteredMemberIdSet.has(memberId)
    ).length;
  ...
  const handleCreateIssuedCoupons = async (): Promise<void> => {
    ...
    const response = await createIssuedCoupons(
  ...
  const handleDeleteIssuedCoupon = async (
  ...
  const handleCouponNameDraftChange = (
    couponId: number,
    value: string
  ): void => {
