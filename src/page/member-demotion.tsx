...
const resolveMemberDemotionErrorMessage = (errorName?: string): string => {
  return resolveAdminErrorMessage(errorName, {
    overrides: memberDemotionErrorOverrides,
  });
};
    ...
    <div className="space-y-5">
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-muted/30 p-4">
          ...
          <Button
            onClick={() => void handleDemoteMembers()}
        ...
        <div className="space-y-3 text-sm">
          ...
          <div>
            ...
            <div className="break-all rounded-lg border bg-background px-3 py-2">
              {lastDemotedStudentIds.length > 0
                ? lastDemotedStudentIds.join(", ")
                : "-"}
            </div>
          </div>
        </div>
