...
const resolveFeatureFlagsErrorMessage = (errorName?: string): string => {
  return resolveAdminErrorMessage(errorName, {
    overrides: featureFlagsErrorOverrides,
  });
};
...
const FeatureFlagsPage: React.FC = () => {
  ...
  const applyFlags = useCallback((nextFlags: AdminFeatureFlags): void => {
    setFlags(nextFlags);
    setMemberSignupEnabled(
      nextFlags.memberSignup.enabled ?? nextFlags.memberSignup.signupAllowed
    );
    setStudyCreationEnabled(
      nextFlags.studyCreation.enabled ??
        nextFlags.studyCreation.studyCreationAllowed
    );
    setEnrollOpenAt(
      toDateTimeLocalValue(
        nextFlags.studyEnrollWindow.openAtRaw ??
          nextFlags.studyEnrollWindow.openAt
      )
    );
    setEnrollCloseAt(
      toDateTimeLocalValue(
        nextFlags.studyEnrollWindow.closeAtRaw ??
          nextFlags.studyEnrollWindow.closeAt
      )
    );
  }, []);
    ...
    <div className="space-y-5">
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            onClick={() => void handleSaveMemberSignup()}
        ...
        <div className="flex flex-wrap gap-2 text-sm">
          <Badge variant="secondary">
            현재: {flags?.memberSignup.signupAllowed ? "허용" : "차단"}
          </Badge>
          <Badge variant="outline">
            유효성: {flags?.memberSignup.valid ? "정상" : "비정상"}
          </Badge>
          <Badge variant="outline">
            원시값: {flags?.memberSignup.rawValue ?? "-"}
          </Badge>
        </div>
      </AdminSectionCard>
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            onClick={() => void handleSaveStudyCreation()}
            disabled={savingKey !== null}
          >
        ...
        <div className="flex flex-wrap gap-2 text-sm">
          <Badge variant="secondary">
            현재: {flags?.studyCreation.studyCreationAllowed ? "허용" : "차단"}
          </Badge>
          <Badge variant="outline">
            유효성: {flags?.studyCreation.valid ? "정상" : "비정상"}
          </Badge>
          <Badge variant="outline">
            원시값: {flags?.studyCreation.rawValue ?? "-"}
          </Badge>
        </div>
      </AdminSectionCard>
      ...
      >
        ...
        <Button
          onClick={() => void handleSaveStudyEnrollWindow()}
          disabled={savingKey !== null}
        >
        ...
        <div className="grid gap-2 text-sm">
          <div>
            현재 적용 시작 시각:{" "}
            {formatDateTime(flags?.studyEnrollWindow.openAt ?? null)}
          </div>
          <div>
            현재 적용 종료 시각:{" "}
            {formatDateTime(flags?.studyEnrollWindow.closeAt ?? null)}
          </div>
          <div>
            현재 신청 가능 여부:{" "}
            {flags?.studyEnrollWindow.enrollmentAllowedNow ? "가능" : "불가"}
          </div>
          <div>
            기간 유효성:{" "}
            {flags?.studyEnrollWindow.valid ? "정상" : "비정상(기본 허용 적용)"}
          </div>
        </div>
      </AdminSectionCard>
    </div>
